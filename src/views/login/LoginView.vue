<template>
  <div class="login">
    <div class="login-con">
      <ul class="menu-tab">
        <li v-for="v in MenuData" :key="v.type" @click="clickMenu(v)" :class="{current:v.current}">
          {{v.txt}}
        </li>
    </ul>
    <!-- 表单部分 -->
    ...
    <el-form-item prop="passwords" v-show="model==='register'">
      <label>重复密码</label>
      <el-input v-model="ruleForm.passwords" type="password" minlength="6" maxlength="15" />
    </el-form-item>

    <el-form-item>
      <el-button type="primary" @click="submitForm(ruleFormRef)" class="login-btn block" :disabled="btnbool">{{model==="login"?'登录':'注册'}}</el-button>
